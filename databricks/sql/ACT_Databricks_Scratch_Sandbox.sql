-- ACT Databricks Scratch Sandbox Schema
--
-- Target catalog/schema:
--   lum_databricks_dev_es1_sandbox.scratch
--
-- AIG provided a sandbox catalog with an existing `scratch` schema rather than
-- separate bronze/silver/gold schemas. This file preserves medallion intent in
-- table names:
--
--   scratch.bronze_*             -> future bronze.*
--   scratch.silver_*             -> future silver.*
--   scratch.gold_lummus_act_*    -> future gold_lummus_act.*
--
-- Run this in Databricks SQL or a SQL notebook after confirming you can create
-- and drop a small test table in `scratch`.

USE CATALOG lum_databricks_dev_es1_sandbox;
USE SCHEMA scratch;
...
CREATE TABLE IF NOT EXISTS bronze_fusionlive_sync_runs (
  run_id STRING,
...
CREATE TABLE IF NOT EXISTS bronze_fusionlive_sync_watermarks (
...
CREATE TABLE IF NOT EXISTS bronze_fusionlive_project_participants_raw (
  run_id STRING,
  fusion_workspace_id STRING,
  fusion_subproject_id STRING,
...
CREATE TABLE IF NOT EXISTS bronze_act_excel_deliverables_raw (
  run_id STRING,
...
CREATE TABLE IF NOT EXISTS silver_fusionlive_documents_latest (
  fusion_workspace_id STRING,
  fusion_subproject_id STRING,
...
CREATE TABLE IF NOT EXISTS silver_fusionlive_project_members (
  fusion_workspace_id STRING,
  fusion_subproject_id STRING,
  fusion_user_id STRING,
  email STRING,
...
CREATE TABLE IF NOT EXISTS silver_fusionlive_reviews_latest (
  fusion_workspace_id STRING,
  fusion_subproject_id STRING,
...
CREATE TABLE IF NOT EXISTS silver_fusionlive_review_recipients_latest (
  fusion_review_id STRING,
  fusion_document_id STRING,
  recipient_user_id STRING,
  recipient_email STRING,
...
CREATE TABLE IF NOT EXISTS silver_act_projects (
  act_project_id STRING,
  project_name STRING,
  fusion_workspace_id STRING,
...
CREATE TABLE IF NOT EXISTS silver_act_project_members (
  act_project_id STRING,
  user_id STRING,
  fusion_user_id STRING,
...
CREATE TABLE IF NOT EXISTS silver_act_deliverables (
...
CREATE TABLE IF NOT EXISTS silver_act_deliverable_matches (
  match_id STRING,
  act_project_id STRING,
  deliverable_id STRING,
  document_reference STRING,
...
CREATE TABLE IF NOT EXISTS silver_act_deliverable_phases (
  deliverable_id STRING,
  act_project_id STRING,
...
CREATE TABLE IF NOT EXISTS gold_lummus_act_dashboard_deliverables (
  deliverable_id STRING,
  act_project_id STRING,
  document_reference STRING,
  title STRING,
...
CREATE TABLE IF NOT EXISTS gold_lummus_act_active_alerts (
  alert_id STRING,
  act_project_id STRING,
  deliverable_id STRING,
  alert_type STRING,
...
CREATE TABLE IF NOT EXISTS gold_lummus_act_alert_history (
  history_id STRING,
  alert_id STRING,
  act_project_id STRING,
  deliverable_id STRING,
...
CREATE TABLE IF NOT EXISTS gold_lummus_act_teams_delivery_queue (
  delivery_id STRING,
  act_project_id STRING,
  alert_id STRING,
  digest_id STRING,
...
CREATE TABLE IF NOT EXISTS gold_lummus_act_daily_digest_queue (
  digest_id STRING,
  act_project_id STRING,
  recipient_user_id STRING,
  recipient_email STRING,
...
CREATE OR REPLACE VIEW gold_lummus_act_vw_active_projects AS
SELECT *
FROM silver_act_projects
WHERE active_auto_refresh = true
  AND refresh_mode = 'automatic';

CREATE OR REPLACE VIEW gold_lummus_act_vw_pending_reviewers AS
SELECT
  r.fusion_review_id,
  ...
  r.status
FROM silver_fusionlive_reviews_latest r
JOIN silver_fusionlive_review_recipients_latest rr
